...
{
    if( argv[2] != NULL && ( strstr(argv[2],".bmp") == ".bmp" ) )             //to check the file are .bmp or not
    {
	...
	strcpy( encInfo->extn_secret_file,strstr(argv[3],".") );             //to store the extn string
    }
    else
    {
	return e_failure;
    }
    if( argv[4] != NULL && ( strstr(argv[4],".bmp") == ".bmp" ) )           //to check the file are .bmp or not
    {
    ...
    { 
	encInfo->stego_image_fname = "stego.bmp";              //the 5th argumnet are not mandatory if you not give auto save stego.bm
	return e_failure;
    }
}
